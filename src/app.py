...
import theme
from ai_model import CLASS_NAMES_VERBOSE, predict_image
...
def main():
    if st.session_state["authenticated"]:
        with st.sidebar:
            ...
            st.markdown("---")
            st.button(
                " Home", on_click=nav_to, args=("home",), use_container_width=True
            )
    ...
    if st.session_state["authenticated"]:
        ...
        pages = {
            ...
            "view_all_appointments": view_all_appointments_page,
            "manage_users": manage_users_page,
            "book_appointment": book_appointment_page,
        }
...
def login_register_page():
    theme.hero(
        "Skin Care Connect",
        "AI-assisted triage for skin lesions, connected to real dermatologists.",
    )
    theme.disclaimer()
...
def home_page():
    ...
    theme.hero(
        ...
        f"Signed in as {role}. Here's what you can do.",
    )
    theme.disclaimer()
    ...
    if is_user() or is_dermatologist():
        features.append(
            (
                "",
                "Analyze Image",
                "Upload a skin photo for an AI-assisted classification.",
            )
        )
    if is_user():
        features.append(("", "History", "Review your past analyses."))
        features.append(
            ("", "Appointments", "Track appointments booked from your analyses.")
        )
    if is_dermatologist() or is_admin():
        features.append(
            ("", "All Appointments", "See and respond to every booked appointment.")
        )
    if is_admin():
        features.append(
            ("", "Manage Users", "Add, edit or remove user accounts and roles.")
        )
...
def analyze_page():
    theme.hero(
        "Analyze Skin Image",
        "Upload a clear, well-lit photo of the skin area of concern.",
    )
    theme.disclaimer()
            ...
            try:
                with st.spinner("Analyzing image..."):
                    ...
                    prediction_id = save_prediction_result(
                        user_id, unique_filename, result
                    )
...
def display_results(result, prediction_id):
    if (
        result["error"]
        and not result.get("is_valid_prediction", False)
        and result["predicted_class"] in ("Non-Skin Image", "Unclassified")
    ):
        theme.card_start()
    ...
    if predicted_class == "Melanoma":
        risk_kind, risk_label = "danger", " Higher-risk class detected"
    elif confidence > 80:
...
def appointments_page():
    ...
    for appt in appointments:
        aid, sched_date, status, notes, derm_notes, pred_class, pred_conf, pred_time = (
            appt
        )
        theme.card_start()
        ...
        st.write(
            f"**Related analysis:** {pred_class} ({pred_conf:.1f}%) from {pred_time}"
        )
        theme.card_end()
    ...
    for appt in appointments:
        ...
        with st.form(key=f"update_form_{aid}"):
            ...
            new_notes = st.text_area(
                "Reply to patient", value=derm_notes or "", key=f"notes_{aid}"
            )
...
def manage_users_page():
    ...
    if users:
        df = pd.DataFrame(
            users, columns=["ID", "Username", "Email", "Role", "Created At"]
        )
            ...
            for index, row in edited_df.iterrows():
                if row["Role"] != df.iloc[index]["Role"]:
                    if update_user_role(row["ID"], row["Role"]):
                        st.success(
                            f"Updated role for user #{row['ID']} to {row['Role']}"
                        )
                        changed = True
                    else:
        ...
        user_ids_to_delete = st.multiselect(
            "Select user IDs to delete", options=df["ID"].tolist()
        )
...
def book_appointment_page():
    theme.hero(
        "Book Appointment",
        "Confirm a slot to review this analysis with a dermatologist.",
    )
    ...
    with st.form(key="appointment_form"):
        ...
        if st.form_submit_button("Confirm appointment"):
            ...
            book_appointment(
                st.session_state["user_id"], pred_id, scheduled_datetime, notes
            )
